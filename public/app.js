...
function saveTask(categoryId, taskId, isGlobal) {
    ...
    if (town && !appState.global.towns[categoryId][town]) { 
        appState.global.towns[categoryId][town] = { order: Date.now(), color: pastelColors[0] }; 
    }
    ...
    if (isTrade) { 
        taskData.type = 'trade'; taskData.fromItem = fromItem; taskData.fromQty = fromQty; taskData.toItem = toItem; taskData.toQty = toQty; 
    } else { 
        taskData.type = 'normal'; taskData.label = label; 
    }
...
function toggleTask(taskId, isChecked, isShared, categoryId) {
    if (isShared) { 
        appState.global.sharedChecks[taskId] = isChecked; 
    } else { 
        appState.characters.find(c => c.id === appState.activeTabId).checks[taskId] = isChecked; 
    } 
    saveData(); 
...
function moveBlock(categoryId, type, idOrName, direction) {
    ...
    const curBlock = blocks[idx]; const tgtBlock = blocks[targetIdx]; 
    const tempOrder = curBlock.order; curBlock.order = tgtBlock.order; tgtBlock.order = tempOrder;
...
function editEvent(id) {
    ...
    appState.characters.forEach(c => { let isSelected = !ev.targetChars || ev.targetChars.includes('all') || ev.targetChars.includes(c.id); chipsHtml += `<div class="char-chip ${isSelected ? 'selected' : ''}" onclick="this.classList.toggle('selected')" data-char-id="${c.id}">${c.name}</div>`; }); 
    chipsHtml += `</div>`;
    ...
    selectedEventDays = [...(ev.days || [])]; editingEventId = id; 
    setTimeout(() => wrap.scrollIntoView({ behavior: 'smooth', block: 'end' }), 10);
}
...
function toggleEventDay(btn, day) { 
    ...
    if (btn.classList.contains('selected')) { 
        if (!selectedEventDays.includes(day)) selectedEventDays.push(day); 
    } else { 
        selectedEventDays = selectedEventDays.filter(d => d !== day); 
    } 
}
...
function addNotice() {
    ...
    document.getElementById('input-notice-url').value = ''; 
    document.getElementById('add-wrap-notice').classList.add('hidden'); 
    saveData(); renderGlobal();
}
function deleteNotice(id) { 
    if(!confirm("이 공지사항을 내역에서 삭제하시겠습니까?")) return; 
    appState.global.notices = appState.global.notices.filter(n => n.id !== id); 
    saveData(); renderGlobal(); 
}
...
function saveCharacterName() { 
    ...
    if (newName) { const char = appState.characters.find(c => c.id === managingCharId); if (char) char.name = newName; saveData(); renderTabs(); } 
    closeCharModal(); 
}
...
function deleteGlobal(type, id) { 
    ...
    appState.global[type] = appState.global[type].filter(i => i.id !== id); 
...
function executeDelete(type) {
    ...
    const { category, taskId, isGlobal } = deleteTarget; 
    const activeChar = appState.characters.find(c => c.id === appState.activeTabId);
    ...
    if (type === 'all') { 
        ...
        appState.characters.forEach(char => { 
            ...
            delete char.checks[taskId]; 
            delete appState.global.sharedChecks[taskId]; 
            if (char.hiddenTasks) char.hiddenTasks = char.hiddenTasks.filter(id => id !== taskId); 
        }); 
    } else if (type === 'local') { 
        if (isGlobal) { 
            if (!activeChar.hiddenTasks) activeChar.hiddenTasks = []; 
            activeChar.hiddenTasks.push(taskId); 
        } else { 
            activeChar.customTasks[category] = activeChar.customTasks[category].filter(t => t.id !== taskId); 
        } 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 헤더 및 글로벌 버튼 연결 (ui.js, data.js의 함수 참조)
